    ...
    def Run(self):
        ...
        with open(self.saveFile, "w") as saveFile:
            for idx, size in enumerate(sizeList):
                ...
                casualData = {"trainX": xnCasual.ravel().tolist(), "trainY": yn.ravel().tolist(), "label": 0,
                              "type": "casual", "size": int(size), "index": idx}
                antiCasualData = {"trainX": yn.ravel().tolist(), "trainY": xnCasual.ravel().tolist(), "label": 1,
                                  "type": "antiCasual", "size": int(size), "index": idx}

                np.random.shuffle(xn)

                independentData = {"trainX": xn.ravel().tolist(), "trainY": yn.ravel().tolist(), "label": 0.5,
                                   "type": "independent-casual", "size": int(size), "index": idx}
                # saving casual
