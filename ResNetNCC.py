    ...
    def Test(self, epoch):
        with torch.no_grad():
            ...
            if acc > self.BestACC:
                self.BestACC = acc
                print("Save Model...")
                torch.save(self.model, './model/ResNet_model_final.pt')
            self.writer.add_scalar("Test Loss", np.mean(loss_list), epoch)
            self.writer.add_scalar("Test Acc", acc, epoch)
            print("epoch:", epoch, "acc:", acc, "loss:", np.mean(loss_list))

    def TestOtherFormat(self, img):
        with torch.no_grad():
            self.model = torch.load('./model/ResNet_model_final.pt').cuda()
            self.model.eval()
            ResNetFeatures = self.resnet_model(img.cuda().float())
            NCCFeatures, logits, _ = self.model(ResNetFeatures)
            NCCFeatures = NCCFeatures.data.cpu()
            logits = logits.data.cpu()
            return NCCFeatures, logits
